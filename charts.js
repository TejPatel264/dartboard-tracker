...
function drawBlankDartboard(canvas) {
    ...
    drawCircle(225.5, 1,"#55555555")
    drawCircle(170,1,"#eee")
    drawCircle(162)
    drawCircle(107)
    drawCircle(99)
    drawCircle(16)
    ...
    for (let i=0; i<20; i++) {
        ...
        let colour = i % 2 == 0 ? "#b1122633" : "#007a3d33"
    ...
    return new Promise(resolve => {
        ...
        function animate() {
            for (let k=0; k<dartsPerFrame && i < throws.length; k++) {
                ...
                const t = throws[i]
                if (t.dist < 6 || (t.dist >162 && t.dist < 170) || (t.dist > 99 && t.dist < 107)) ctx.strokeStyle = "#d4a017";
...
export function createHeatmap(dartboardCanvas, dartMarkerCanvas, heatmapCanvas, sessions) {
    ...
    page.addEventListener("transitionend", async () => {
        await drawAllDarts(dartMarkerCanvas, sessions);
        drawHeatmap(heatmapCanvas,sessions)
        setTimeout(() => {
            dartMarkerCanvas.style.opacity = 0;
            dartMarkerCanvas.addEventListener("transitionend", () => {
